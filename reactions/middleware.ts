...
const isFreetExists = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!freet) {
    res.status(404).json({
      error: {
        freetNotFound: `Freet with freet ID ${req.body.id as string} does not exist.`
      }
    });
    return;
  }
...
const hasUserAlreadyReacted = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (reaction) {
    res.status(403).json({
      error: {
        freetNotFound: 'You have already reacted to this Freet.'
      }
    });
    return;
  }
...
const hasUserLiked = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (reaction && !reaction.liked) {
    res.status(404).json({
      error: {
        freetNotFound: 'You have not liked this Freet.'
      }
    });
    return;
  }
  ...
  if (!reaction) {
    res.status(403).json({
      error: {
        freetNotFound: 'You have not reacted to this Freet.'
      }
    });
    return;
  }
...
const hasUserDisliked = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (reaction && reaction.liked) {
    res.status(404).json({
      error: {
        freetNotFound: 'You have not disliked this Freet.'
      }
    });
    return;
  }
